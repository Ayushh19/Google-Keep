...
const EditLabels = () => {
  ...
  const handleAddLabel = async () => {
    ...
    try {
      ...
      if (response.data?.status?.success) {
        fetchLabels(); // Refresh to get the actual data from server
      }
    } catch (error) {
      console.error("Error adding label:", error);
      // Remove the temporary label if the API call fails
      setLabels((prevLabels) => prevLabels.filter((label) => label.id !== tempId));
    } finally {
  ...
  const handleSaveEdit = async () => {
    if (!editingLabelText.trim() || !editingLabelId) return;
    ...
    try {
      ...
      const response = await axios({
        method: "post",
        url: `https://fundoonotes.incubation.bridgelabz.com/api/noteLabels/${editingLabelId}/updateNoteLabel`,
        headers: {
          Authorization: token,
          "Content-Type": "application/json",
        },
  ...
  const handleDeleteLabel = async () => {
    ...
    // Optimistically update UI
    setLabels((prevLabels) => 
      prevLabels.filter((label) => label.id !== labelToDelete.id)
    );
    ...
    try {
      ...
      const response = await axios({
        method: "delete",
        url: `https://fundoonotes.incubation.bridgelabz.com/api/noteLabels/${labelToDelete.id}/deleteNoteLabel`,
        headers: {
          Authorization: token,
        },
      });
